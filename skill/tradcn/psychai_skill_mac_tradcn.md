# PsychAI — Claude Code Skill（macOS 版·繁中版）
# 作者：伪63
# 文件名：psychai.md
...
本版本為 macOS 試運行版，相較 Windows 完備版**目前缺失以下功能**：

- **沒有 WeFlow 微信解密整合**：Mac 版暫未對接 WeFlow（GitHub 上 `hicccc77/WeFlow` 項目據稱提供 Mac 版本，作者尚未實測；後續會補齊對應章節）
- **微信聊天記錄只能手動接入**：使用者需在另一台 Windows 電腦上用 WeFlow 匯出 JSON 後傳到 Mac，或直接手動整理對話文字
- **wxid 全程需使用者手動提供**：沒有自動從設定檔 / API 獲取的路徑

不影響的功能（與 Windows 版完全一致）：錄音轉寫分析、日記/手寫文字分析、PDF 文件解析、Pages 文件解析（textutil）、11 大心理學框架、動態問卷、跨會話連續檔案。
...
首先確認工作目錄是否存在。使用者的 PsychAI 工作目錄為 `~/psychai`（即 `/Users/[使用者名稱]/psychai/`）。

實際路徑每次運行都直接用 `Path.home() / 'psychai'` 計算（無需儲存到檔案中，因為 `Path.home()` 在同一台機器上始終回傳相同值）。後續所有檔案操作均以此為根目錄。**不要硬編碼 `~/...`**——一律用 Python 解析後的絕對路徑。
...
~/psychai/
  ...
  tools/
    extract_text.py         ← 首次運行自動建立，用於提取 docx/pages 文字
  .state.json
```

需要檢查的核心檔案（路徑均以 `work_dir` 為根）：
- `{work_dir}/analysis/profile_core.md` — 人格核心（不存在則為首次運行）
...
- `{work_dir}/.state.json` — 狀態記錄（對話輪次、檔案版本、上次運行時間）

---

## 第二節：判斷運行模式

### 0. Python 可用性探測（必須先於任何 `python3` 指令執行）

無論模式一還是模式二，第一步都必須先驗證 `python3` 真實可用——macOS 11 (Big Sur) 後系統不再預裝 `python`，需要使用者從 python.org 下載或透過 Homebrew 安裝。即便已安裝，PATH 中可能指向不可用版本。
...
- `pyOk = false` → 根據 `pyReason` 給出針對性提示，告知使用者後**立即終止本次運行**：

| `pyReason` | 文案 |
|------|------|
| `not_found` | 我需要 Python 3 但你的系統裡找不到 `python3` 指令。可以從 [python.org](https://www.python.org/downloads/) 下載（3.8 以上），或終端機執行 `brew install python3`（前提是裝了 Homebrew）。裝完重新運行 `/psychai`。 |
| `too_old:X.Y` | 偵測到 Python X.Y，但 PsychAI 需要 3.8 以上版本（用到 f-string 等語法）。在終端機執行 `brew upgrade python3`（Homebrew）或去 python.org 下載最新版。conda 使用者：`conda update python`。 |
| `unrecognized` | 偵測到 `python3` 指令但版本輸出格式異常（輸出：[貼上 $pyVer]）。可能是非標準 Python 發行版。請嘗試 `python3 --version` 看實際輸出，發給我。 |

如果使用者暫時不想裝/升級 Python，可以提示：可以先把檔案在 Pages/Word 裡另存為 .txt 放入對應資料夾——但首次運行的目錄建立仍需要 Python，至少要裝一次。

**模式二專屬補充檢查**：若 `.state.json` 已存在但 `python3` 突然不可用（使用者在兩次會話之間解除安裝/改 PATH），按上述告知使用者重裝，不要嘗試任何後續 `python3 -c` 呼叫。
...
if not p.exists():
    ...
    try:
        ...
        # 損壞：重新命名為 .state.json.broken 後當作首次運行
        p.rename(p.with_suffix('.json.broken'))
        print('first_run')
"
```

**損壞處理邏輯**：若 `.state.json` 存在但 JSON 解析失敗（手動編輯寫壞、磁碟損壞、編碼錯誤等），自動重新命名為 `.state.json.broken` 保留現場，然後走首次運行流程重建。開場時告知使用者：「偵測到 `.state.json` 損壞（已備份為 `.state.json.broken`），將重新初始化。已有的 analysis/ 檔案不受影響。」

### 模式一：首次運行（.state.json 不存在 或 已損壞被重置）

執行以下順序：

**1. 建立全部目錄和工具檔案**

**建立目錄結構**（Python，Windows / Mac / Linux 通用）：

**重要**：macOS 系統自帶的 Python 指令是 `python3`，而非 `python`（系統 `python` 預設不存在）。本文件所有指令統一使用 `python3`；若使用者透過 pyenv / Homebrew / Anaconda 安裝了 `python` 別名也可使用。
...
支援：.docx（Word）/ .pages（macOS Pages）/ .pdf / .txt / .md
用法：python3 extract_text.py <輸入檔案路徑> <輸出txt路徑>
"""
...
def extract_pages(path: str) -> str:
    """使用 macOS 內建 textutil 指令（支援新舊版 Pages）。"""
...
def extract_pdf(path: str) -> str:
    """
    PDF 文字層提取：優先 pdfplumber，不可用時 fallback 到 macOS 內建 textutil。
    掃描件無文字層會報錯提示使用者。
    """
    try:
        ...
        if not pages_text:
            raise ValueError(
                ...
                "請將 PDF 用 Pages/Word 打開並另存為 .docx 後重新上傳。"
            )
        return "\n\n".join(pages_text)
    except ImportError:
        # fallback 到 macOS 內建 textutil
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("用法：python3 extract_text.py <輸入檔案> <輸出txt>")
        sys.exit(1)
...
```
- 若安裝失敗，extract_text.py 會自動 fallback 到 macOS 內建 `textutil`（處理文字型 PDF 足夠）
...
欄位說明：
- `wechat_last_read`：key 為聯絡人 wxid 或群 chatroom id，value 為上次讀取的最新訊息時間戳（Unix 秒），用於增量分析。
- `user_wxid`：使用者本人微信 wxid，用於 is_me 判斷。
- `summary_mode`：是否維護 profile 檔案頂部摘要塊（預設 true；使用者說「關掉摘要」時設為 false，摘要維護跳過，每次運行讀全文）。
...
告知使用者：
> macOS 沒有像 Windows 上 WeFlow 那樣的微信本地資料庫解密工具。如果你想分析微信聊天記錄，有兩種方式：
> 1. 在另一台 Windows 電腦上用 WeFlow 匯出 JSON，傳到這台 Mac 上放入 `input/wechat/`
> 2. 手動整理對話（複製貼上成 txt 或 md），放入 `input/wechat/` 或 `input/diary/`
>
> 其他材料（錄音轉寫、日記、PDF 等）不受影響，正常放入對應資料夾即可。

**wxid 設定**（僅當使用者有微信 JSON 時需要）：
> 為了識別哪些訊息是你說的，請告訴我你的微信 wxid。
> 在另一台 Windows 電腦上打開 WeFlow，在介面裡找一下你自己的 wxid——一串以 `wxid_` 開頭的字元。複製後發給我就行。
> 一次告訴我就好，之後我會記住。

驗證：使用者回覆必須以 `wxid_` 開頭，否則要求重新提供。寫入 `.state.json` 的 `user_wxid` 欄位。
...
### 模式二：再次運行（.state.json 存在）

**1. 確定工作目錄並讀取現有檔案**

工作目錄與首次運行相同，直接計算（無需從檔案中讀取）：
```python
python3 -c "from pathlib import Path; print(Path.home() / 'psychai')"
```
...
4. `{work_dir}/.state.json` — 讀取運行狀態（sessions / questionnaire_done / wechat_last_read 等）
...
- 使用者確認後再進入第五節讀取流程
- 若發現單個檔案 > 5MB（如錯放的視頻/壓縮包），單獨詢問是否跳過

**3. 檢查微信 JSON 新訊息**

若 `input/wechat/` 下有 JSON 檔案，對每個檔案：
- 讀取 JSON，識別聯絡人 id（從檔案名或 JSON 結構中提取）
- 過濾出 `timestamp > wechat_last_read[id]` 的新訊息
...
> 歡迎回來。[情況彙整，選擇適用的：]
> - 發現 input/ 有 [N] 個新檔案
> - 微信 JSON 裡讀到 [N] 條新訊息（[時間範圍]）
> - 沒有新內容——你可以直接和我聊，或者把新材料放入 input/ 後重新運行 /psychai。
...
| `.pages` | ✅ | 內建 textutil 指令（新舊版 Pages 全覆蓋） |
| `.pdf`（文字層）| ✅ | pdfplumber；不可用時 fallback 到 textutil |
| `.pdf`（掃描件）| ❌ | 提示使用者用 Pages/Word 打開另存為 .docx |
| `.json` | ✅ | 用於結構化資料（如手動整理的聊天記錄） |
| 音訊檔案（.mp3/.m4a/.wav）| ❌ | **不支援**——recordings/ 資料夾只接受**已轉寫為 txt/md 的文字**，需先用其他工具轉寫（如 macOS 備忘錄的即時轉寫、Whisper） |
| 圖片（.jpg/.png/截圖）| ⚠️ | 當前 skill 不讀 input/ 下的圖片；使用者可直接在對話裡貼上圖片，Claude 視覺能力會讀 |

**通用規則**：`.docx` / `.pages` / `.pdf` 必須先複製到 ASCII 暫存路徑（如 `/tmp/`），再用 `tools/extract_text.py` 提取（中文路徑會導致 Bash 讀取失敗）。
...
```
- 輸出 `OK` → 正常使用
- 輸出 `NEEDS_REBUILD` → 腳本誤刪 / 被截斷 / 損壞 → 觸發首次運行的「建立 tools/extract_text.py」流程重新寫入（第一節中已定義完整腳本內容）；告知使用者：「文字提取腳本不存在，已自動重建。」

### docx / pages / pdf 提取指令

```bash
cp "原始路徑/文件.docx" /tmp/temp_input.docx
python3 ~/psychai/tools/extract_text.py /tmp/temp_input.docx /tmp/temp_output.txt
```

`.pages` 和 `.pdf` 檔案同理，直接傳入腳本，腳本根據副檔名自動選擇提取分支。

### 錄音/轉寫檔案的特殊規則

**關鍵澄清（首次運行時必須告知使用者）**：`input/recordings/` 資料夾**只接受已轉寫為文字的 .txt/.md 檔案**，不接受 .mp3/.m4a/.wav 等音訊檔案本身。skill 本身不做語音轉文字，使用者需要先用其他工具（手機錄音備忘錄的轉寫功能、剪映、Whisper 等）把語音轉成文字，再放入該資料夾。

- 若掃描 `input/recordings/` 時發現音訊檔案（副檔名為 mp3/m4a/wav/aac/flac/ogg/wma），跳過該檔案並提醒使用者：
  > 我發現 [檔案名] 是音訊檔案，但我目前只能讀文字。請把它先轉寫成文字（手機錄音備忘錄、剪映、Whisper 等工具都行），存成 .txt 或 .md，再放回 recordings/ 資料夾。
...
- **時間維度引導**：接入聊天記錄時，主動詢問覆蓋時間跨度：
  ...
  若時間跨度超過 3 個月，按時間段對比分析（前期/後期變化），而非只彙整整體特徵；若使用者指出明確節點（「分手前後」、「高考前後」等），以該節點為分割進行對比
- **非文字訊息處理**：WeChat 匯出中常見 `[圖片]`、`[語音]`、`[視頻]`、`[檔案]`、`[表情]`、`[位置]` 等佔位文字。這些訊息當前**不分析其內容**，但分析時**必須保留其時序位置**，以免錯位理解上下文。可在分析輸出中標注：「該時段存在 N 條非文字訊息未分析，可能影響理解。」
- **其他微信匯出工具**：除 WeFlow 外，以下工具匯出的檔案同樣可放入 `input/wechat/`，PsychAI 會嘗試解析，格式無法識別時明確告知使用者：
  ...
  - **WeChatMsg**（GitHub 開源項目）：支援 CSV / JSON / HTML 多種格式，推薦選 JSON 匯出
...
>
> 一共 6 個話題，中途隨時可以停——下次運行 `/psychai` 會從還沒聊到的部分繼續。

**第一題**：固定從家庭域開始（依戀風格的根源在此，是最重要的底層信號）。
...
**中途退出處理**：使用者在問卷中途結束對話（無論是顯式說「先到這」還是直接關閉）→ 不強求完成，已記錄的 `questionnaire_progress` 自然保留；下次運行時從未覆蓋域恢復。
...
> **最有價值**：如果你有錄音或語音轉文字的文稿（對自己說話的錄音、和他人對話的錄音，或任何你私下說話的記錄），放入 `input/recordings/` 資料夾，然後重新運行 `/psychai`。錄音裡你是最不設防的。
...
macOS 沒有原生的微信本地資料庫解密工具。兩條可行路徑：

> **路徑一（推薦）**：在另一台 Windows 電腦上用 WeFlow 把對應聯絡人/群聊匯出為 JSON，傳到 Mac 上放入 `input/wechat/`。

> **路徑二**：直接從微信裡複製對話文字到一個 .txt/.md 檔案，放入 `input/wechat/` 或 `input/diary/`。這種方式不會有時間戳，分析會更粗，但仍然有用。
...
- **焦慮-矛盾型**：過度啟動依戀；恐懼被遺棄；尋求持續確認；對方沉默時焦慮啟動
- **回避型**：停用依戀系統；過度自立；對依賴感不適；被需要時退縮
...
三種應對方式：順從（按圖式行事）、回避（回避觸發情境）、過度補償（做與圖式相反的事）
...
2. **先觀察再診斷**：先從文字/行為證據出發，再映射框架。多鏡頭交叉
3. **區分循證心理學與流行心理學**：明確說出引用的內容是否來自同行評審研究，不把流行心理學概念當臨床框架使用
4. **使用者糾正優先，但強證據矛盾時須標注**：使用者說「不對」時預設接受並更新——除非使用者的否認與文字中的大量具體行為證據直接矛盾，且矛盾程度已超出合理自我認知偏差的範圍。此時不沉默地接受，而是明確指出矛盾：「我看到的證據指向X，你說的是Y，這兩者之間有很大落差，我想先理解這個落差再更新。」典型情形：聊天記錄顯示對某人極度依賴，使用者卻聲稱完全不在乎對方；行為證據顯示強烈的控制欲，使用者卻說自己毫不在意結果。**自我欺騙和防禦性否認本身就是分析對象，不是更新指令。**
...
**讀取**：每次運行時優先讀各 profile 檔案的 `## 摘要` 塊，再根據當次對話的相關性決定是否讀取完整內容。
**關閉**：使用者說「關掉摘要」時，將 `summary_mode` 設為 false，此後寫入不更新摘要塊，讀取直接讀全文；除非使用者主動提出，不建議關閉。

### AI每次運行時的讀取順序

每次 `/psychai` 運行，按以下順序讀取，再開始本次分析：
...
4. .state.json
   → 確認 sessions 數、wechat_last_read、questionnaire_done 等運行狀態
```
...
觸發條件：使用者提供的 wxid 不符合 `wxid_` 開頭格式，或拒絕提供

> 我需要知道你的微信帳號 ID（wxid），才能判斷哪些聊天訊息是你說的。
> 在另一台 Windows 電腦上打開 WeFlow，在介面裡找一下你自己的 wxid——一串以 `wxid_` 開頭的字元。複製後發給我。
> 不確定的話把你看到的都發給我，我來判斷。
> 如果你暫時不需要分析微信記錄，可以直接告訴我「跳過」，wxid 之後再說。
...
本 skill 的方法論體系由**伪63**設計與開發，基於其個人心理分析項目的實踐經驗構建。
