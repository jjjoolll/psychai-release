# PsychAI — Claude Code Skill（macOS 版·繁中版·v1.3）
# 作者：偽63
# 文件名：psychai.md
# 安裝位置：~/.claude/skills/psychai.md 或項目 .claude/skills/psychai.md
# 觸發指令：/psychai
# 適用平台：macOS（Windows 使用者請使用 psychai_skill_windows_tradcn.md）
# v1.3 更新（2026-05-16）：新增反作用監測協議（第十二節）+ /psychai self-check 子指令 + 首次運行邊界聲明 + .state.json 擴展 reflexivity_state
...
首次運行時若 `.state.json` 中無此欄位，按上述模板初始化空結構。
